...
df_results <- lapply(seq(from = 5, to = 50, by = 5), function(buffer){
  ...
  # Full Models ----------------------------------------------------------------
  ## Round 2-5
  lib_dem_val_index.full.lm               <- felm(as.formula(paste0("lib_dem_val_index ~               completed_near_china.pl10.",buffer,"km.bin + planned_near_china.pl10.",buffer,"km.bin + ", IVs_china," | ",FEs," | 0 | ", CLUSTER_VAR)), data=df[(df$sample_full %in% T) & (df$afro.round %in% 2:5),]) 
  
  ## Round 4
  china.help.country.full.lm              <- felm(as.formula(paste0("china.help.country ~              completed_near_china.pl10.",buffer,"km.bin + planned_near_china.pl10.",buffer,"km.bin + ", IVs_china," | ",FEs," | 0 | ", CLUSTER_VAR)), data=df[(df$sample_full %in% T) & (df$afro.round %in% 4),]) 
  usa.help.country.full.lm                <- felm(as.formula(paste0("usa.help.country ~                completed_near_china.pl10.",buffer,"km.bin + planned_near_china.pl10.",buffer,"km.bin + ", IVs_china," | ",FEs," | 0 | ", CLUSTER_VAR)), data=df[(df$sample_full %in% T) & (df$afro.round %in% 4),]) 
  ...
  # Restricted Models ----------------------------------------------------------
  ## Round 2-5
  lib_dem_val_index.restricted.lm              <- felm(as.formula(paste0("lib_dem_val_index ~              completed_near_china.pl10.",buffer,"km.bin + planned_near_china.pl10.",buffer,"km.bin + completed_near_usaid.",buffer,"km.bin + planned_near_usaid.",buffer,"km.bin + ", IVs_china_usaid," | ",FEs," | 0 | ", CLUSTER_VAR)), data=df[(df$sample_restricted %in% T) & (df$afro.round %in% 2:5),]) 
  ...
  # Restricted US/UK Models ----------------------------------------------------
  ## Round 2-5
  lib_dem_val_index.restrictedusuk.lm <- felm(as.formula(paste0("lib_dem_val_index ~ completed_near_ukaid.",buffer,"km.bin + planned_near_ukaid.",buffer,"km.bin + completed_near_usaid.",buffer,"km.bin + planned_near_usaid.",buffer,"km.bin + ", IVs_china_usaid," | ",FEs," | 0 | ", CLUSTER_VAR)), data=df[(df$sample_restricted_uk %in% T) & (df$afro.round %in% 2:5),]) 
  ...
  coef_full_df <- bind_rows(
    ...
    extract_coefs(china.help.country.full.lm) %>% mutate(model = "Believes China\nHelps Country"),
    extract_coefs(usa.help.country.full.lm) %>% mutate(model = "Believes US\nHelps Country")
  ) %>%
